...
with open(out, 'w', newline='') as outfile:
    ...
    for appendix in datasetNames: # 注意appendix自带.csv
        ...
        N=-1 # 代表读每个文件的全部行
        for eParam in [0,5000,10000_0000]:
            lastParam = eParam
            for seed in range(seedRange):
                print('------------')
                print('seed', seed)
...
# 6: 1/time
c=4
df_perf = pd.read_csv(out, usecols=[0,1,c])
...
print(df_perf)
draw_cd_diagram(df_perf=df_perf,colors=colorsmap,outPath=os.path.join(pngDir,"benchUCR"))
